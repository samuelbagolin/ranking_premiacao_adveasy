    ...
    <div className="space-y-6">
      <div className="flex items-end justify-between border-b border-white/10 pb-4">
        <div>
          ...
          <p className="text-[10px] font-black text-blue-400 tracking-[0.4em] uppercase">Global Sync // Active</p>
        </div>
      ...
      <div className="space-y-4">
        {sorted.map((entry, index) => {
          const borderColorClass = entry.borderColor || 'border-slate-700';
          const glowClass = entry.glowColor || '';
          const textColorClass = entry.color || 'text-slate-400';
          
          return (
            <div 
              key={entry.employeeId}
              className={`group relative overflow-hidden rounded-xl border-l-4 transition-all duration-500 p-1 ${borderColorClass} bg-slate-900/60 hover:bg-slate-800/80 hover:translate-x-2`}
            >
              <div className="flex items-center justify-between p-3">
                <div className="flex items-center gap-6">
                  ...
                  <div className="relative group-hover:scale-110 transition-transform duration-500">
                    <div className={`absolute -inset-1 rounded-full blur-md opacity-40 transition-opacity group-hover:opacity-100 ${borderColorClass.replace('border-', 'bg-')}`}></div>
                    <img 
                      src={entry.avatar} 
                      alt={entry.name}
                  ...
                  <div>
                    <h3 className={`font-orbitron font-black text-2xl leading-none uppercase tracking-tighter text-white ${glowClass}`}>
                      {entry.name}
                    </h3>
                    <div className="flex items-center gap-2 mt-1">
                      <span className={`text-[10px] font-black uppercase px-1.5 py-0.5 rounded bg-white/5 border border-white/10 ${textColorClass}`}>
                          {entry.sector}
                      </span>
                ...
                <div className="text-right">
                  <p className={`font-orbitron font-black text-4xl italic tracking-tighter ${textColorClass} ${glowClass}`}>
